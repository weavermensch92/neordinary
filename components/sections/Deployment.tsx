...
import { PROJECTS, Project } from '../studio/GalleryData';
import { AdvancedGalleryView } from '../studio/AdvancedGalleryView';
...
export const Deployment = ({ onNavigate, onTogglePause, isActive, isExiting, exitDirection }: { onNavigate?: (index: number) => void, onTogglePause?: (paused: boolean) => void, isActive?: boolean, isExiting?: boolean, exitDirection?: 'left' | 'right' }) => {
  ...
  // Exit variants for Deployment elements
  const headerVariants = {
    visible: { y: 0, opacity: 1 },
    exit: { y: -100, opacity: 0, transition: { duration: 0.5, ease: [0.7, 0, 0.3, 1] as any } }
  };

  const leftColVariants = {
    visible: { x: 0, opacity: 1 },
    exit: (i: number) => ({
      x: window.innerWidth, // Move to RIGHT (away from line)
      opacity: 0,
      transition: {
        duration: 0.8,
        delay: i * 0.1,
        ease: [0.7, 0, 0.3, 1] as any
      }
    })
  };
  ...
  return (
    ...
    >
      <AnimatePresence mode="wait">
        {!selectedProject && (
          <motion.div
            key="deployment-content"
            initial="visible"
            animate="visible"
            exit="exit"
            className="flex flex-col gap-16 w-full pb-32"
          >
            <motion.p
              custom={0}
              variants={leftColVariants}
              className="text-2xl md:text-3xl font-black text-white/60 tracking-tighter uppercase max-w-4xl border-l-[12px] border-accent pl-8 py-4 bg-black stagger-item"
            >
            ...
            <motion.div
              ...
              custom={1}
              variants={leftColVariants}
              className="bg-white p-12 lg:p-16 border-l-[2rem] border-accent relative group overflow-hidden stagger-item"
            >
              ...
              <div className="relative z-10 w-full">
                <div className="flex flex-col xl:flex-row justify-between items-start gap-8 mb-16">
                  <h3 className="text-6xl lg:text-8xl font-black text-black uppercase tracking-tighter leading-[0.85]">
                    AI <span className="text-accent underline decoration-8 underline-offset-8">HACKATHONS</span>
                  </h3>
                ...
                <button
                  className="mt-16 bg-black text-white px-8 py-6 flex justify-between items-center w-full max-w-2xl hover:bg-accent hover:text-black transition-colors border-4 border-black group/btn relative z-10 shadow-[8px_8px_0_0_#FF1F1F]"
            ...
            <motion.div
              ...
              className="bg-[#111] p-12 lg:p-16 border-t-[2rem] border-white relative overflow-hidden group stagger-item"
            >
              ...
              <div className="relative z-10 w-full">
                <div className="flex flex-col xl:flex-row justify-between items-end gap-12 border-b-[1rem] border-white pb-12 mb-16">
                  <h3 className="text-6xl lg:text-8xl font-black text-white uppercase tracking-tighter leading-[0.85]">
                    <span className="whitespace-nowrap">DEMO DAY</span><br />
                ...
                <div className="mt-20 w-full bg-accent text-black p-12 border-8 border-white flex flex-col md:flex-row justify-between items-center xl:items-end gap-12 shadow-[16px_16px_0_0_#FFF]">
                  ...
                  <button
                    className="bg-black text-white px-8 py-6 flex items-center justify-between min-w-[21.25rem] hover:bg-white hover:text-black transition-colors border-4 border-black group/btn shrink-0"
            ...
            <motion.div
              ...
              custom={3}
              variants={rightColVariants}
              className="w-full bg-black p-12 lg:p-16 border-8 border-accent relative overflow-hidden text-white flex flex-col gap-10 stagger-item"
            >
              ...
              <div className="flex flex-col xl:flex-row gap-16 items-stretch relative z-10">
                ...
                <div className="bg-white relative w-full xl:w-[280px] aspect-square overflow-hidden flex flex-col stagger-item border-r-[8px] border-b-[8px] border-white/10">
                  {/* Text Area - Top Right */}
      ...
      {selectedProject && createPortal(
        <AdvancedGalleryView project={selectedProject} onBack={() => setSelectedProject(null)} />,
        document.body
      )}
    </SectionLayout>
  );
